...
object WearAlarmPaths {
    ...
    /** Payload: `mmol|thresholdMmol|snoozeMinutes|kind|TEST?` (kind optional, defaults HYPO). */
    fun encodeRing(
        mmol: Double,
        ...
        val base = "$mmol|$thresholdMmol|$snoozeMinutes|$kind"
    ...
    fun decodeRing(data: ByteArray): RingPayload? {
        ...
        return RingPayload(mmol, threshold, snooze, kind, test)
    }
